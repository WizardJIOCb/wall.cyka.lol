@echo off
REM Script to verify that the fixed migrations were applied correctly

echo Verifying fixed migrations...

REM Check if open_count column exists in posts table
...
REM Check if like_count column exists in posts table
...
REM Check if idx_posts_search index exists
...
REM Check if idx_walls_search index exists
...
REM Show all indexes on posts table
...
REM Show all indexes on walls table
